		...
		<div className="p-4 space-y-4">
			...
			<div>
				<button
					...
					className={`bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded ${
						!currentWallet && "opacity-50 cursor-not-allowed"
					}`}
				>
			...
			{signatureDetails && (
				<div className="mt-4 p-4 border rounded-md">
					...
					<p>
						<strong>Sender:</strong> {signatureDetails.sender}
					</p>
					<p>
						<strong>Signer:</strong> {signatureDetails.signer}
					</p>
					<p>
						<strong>Nonce:</strong> {signatureDetails.nonce}
					</p>
					<p>
						<strong>Contract Address:</strong>{" "}
						{signatureDetails.contractAddress}
					</p>
					<p>
						<strong>Wallet Type:</strong>{" "}
						{signatureDetails.isMultiSig ? "Multi-Signature" : "Standard"}
					</p>
				</div>
			)}
			{apiResponse && (
				<div className="mt-4 p-4 border rounded-md">
					...
					{apiResponse.error_code && (
						<p>
							<strong>Error Code:</strong> {apiResponse.error_code}
						</p>
					)}
					<p>
						<strong>Request Type:</strong> {apiResponse.request_type}
					</p>
					<p>
						<strong>Signature:</strong> {apiResponse.signature}
					</p>
				</div>
			)}
			{error && (
